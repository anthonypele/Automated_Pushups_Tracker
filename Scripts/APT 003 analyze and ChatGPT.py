...
for line in lines:
    ...
    if match:
        ...
        if start_of_last_week <= date <= end_of_last_week:
            lines_last_week.append(line)
        elif start_of_week_before_last <= date <= end_of_week_before_last:
            lines_week_before_last.append(line)

pyperclip.copy(f'Please analyze following data:/n Last week: {lines_last_week}/n Week before the last: {lines_week_before_last}')
paste(pyperclip.paste)
# Open ChatGPT and paste data

# Create file in separate folder with the full message from last week stat
